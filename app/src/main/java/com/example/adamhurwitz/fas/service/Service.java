...
public class Service extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        String[] serviceArray = intent.getStringArrayExtra("service_extra");
        String firstParam = serviceArray[0];
        String secondParam = serviceArray[1];
        ...
        try {
            if (!secondParam.equals("popular")) {
                // Construct the URL to fetch data from and make the connection.
                Uri builtUri = Uri.parse(FAS_API_BASE_URL).buildUpon()
                        .appendQueryParameter(SORT_PARAMETER, firstParam)
                        .appendQueryParameter(secondParam, "true")
                        .build();
